...
public class Main {
    ...
    public static List<int[][]> getDirections(int type) {
        ...
        switch (type) {
            case 1 -> {
                for (int i = 0; i < 4; i++) {
                    result.add(new int[][]{directions[i]});
                }
            }
            case 2 -> {
                result.add(new int[][]{directions[0], directions[2]}); // 우, 좌
                result.add(new int[][]{directions[1], directions[3]}); // 하, 상
            }
            case 3 -> {
                for (int i = 0; i < 4; i++) {
                    result.add(new int[][]{directions[i], directions[(i + 1) % 4]});
                }
            }
            case 4 -> {
                for (int i = 0; i < 4; i++) {
                    result.add(new int[][]{directions[i], directions[(i + 1) % 4], directions[(i + 2) % 4]});
                }
            }
            case 5 -> result.add(new int[][]{directions[0], directions[1], directions[2], directions[3]});
        }
        return result;
    }
